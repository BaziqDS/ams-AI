...
const useTypedStream = useStream<
  StateType,
  {
    ...
    CustomEventType: UIMessage | RemoveUIMessage | FrontendActionEvent;
  }
>;
...
const StreamSession = ({
  ...
  const streamValue = useTypedStream({
    ...
    onCustomEvent: (event, options) => {
      ...
      // registered in the parent app (for example, patching the active form).
      const evt = event as unknown as FrontendActionEvent;
      if (evt && evt.type === "frontend_action" && evt.name) {
        console.log("[copilot/stream] dispatching to bridge:", evt.name, evt.args);
        copilotBridge
          .callAction(evt.name, evt.args)
          .then((result) =>
            console.log("[copilot/stream] action result:", evt.name, result),
          )
          .catch((err) => {
            console.warn("[copilot/stream] frontend action failed:", err);
