  ...
  return (
    <div className="w-full min-w-0 max-w-full overflow-hidden rounded-lg border">
      <table className="w-full table-fixed border-collapse">
        <thead>
          <tr>
            <th className="break-all bg-gray-100 px-2 py-0 text-left text-sm" colSpan={2}>
              {toolCall.name}
            </th>
          </tr>
        </thead>
          ...
          {Object.entries(toolCall.args).map(([key, value]) => {
            ...
            return (
              <tr key={key} className="border-t">
                <td className="w-1/3 break-all px-2 py-1 text-xs font-medium">{key}</td>
                <td className="break-all px-2 py-1 font-mono text-xs">{valueStr}</td>
              </tr>
            );
          })}
        </tbody>
