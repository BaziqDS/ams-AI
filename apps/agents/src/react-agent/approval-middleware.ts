import {
  humanInTheLoopMiddleware,
  type HumanInTheLoopMiddlewareConfig,
} from "langchain";

export const FORM_SUBMIT_APPROVAL: HumanInTheLoopMiddlewareConfig = {
  interruptOn: {
    request_form_submit: {
      allowedDecisions: ["approve", "reject"],
      description:
        "Review and approve this AMS form action. It will run against the currently open form in the browser using your signed-in permissions and may create, update, submit, or advance workflow records.",
    },
  },
};

export const formSubmitApprovalMiddleware =
  humanInTheLoopMiddleware(FORM_SUBMIT_APPROVAL);
